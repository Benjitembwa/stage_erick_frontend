...
const Grades = () => {
  ...
  const getFilteredCourses = () => {
    ...
    return courses.filter(
      (course) =>
        course.promotionId === formData.promotionId &&
  ...
  const getFilteredStudents = () => {
    ...
    return students.filter(
      (student) => student.promotionId === formData.promotionId
    );
  };
  ...
  useEffect(() => {
    ...
    if (selectedPromotion) {
      result = result.filter(
        (grade) => grade.promotionId === selectedPromotion
      );
    }
    ...
    if (selectedCourse) {
      result = result.filter((grade) => grade.courseId === selectedCourse);
    }
      ...
      setGrades((prev) =>
        prev.map((grade) =>
          grade.id === currentGrade.id
            ? { ...grade, ...formData, grade: gradeValue }
            : grade
        )
      );
    }
  ...
  // Obtenir le nom complet d'un étudiant
  const getStudentName = (studentId) => {
    const student = students.find((s) => s.id === studentId);
    return student ? `${student.lastName} ${student.firstName}` : "Inconnu";
  };

  // Obtenir le nom d'un cours
  const getCourseName = (courseId) => {
    const course = courses.find((c) => c.id === courseId);
    return course ? course.name : "Inconnu";
  };
            ...
            <div className="relative">
              ...
              >
                ...
                {courses
                  .filter(
                    (c) =>
                      c.promotionId === selectedPromotion &&
                  ...
                  filteredGrades.map((grade) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                          {getPromotionName(grade.promotionId)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                          S{grade.semester}
                        </td>
